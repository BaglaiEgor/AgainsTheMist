...
public class BeaconUpgrade : MonoBehaviour
{
    ...
    public bool TryUpgrade(Inventory inventory)
    {
        ...
        {
            Debug.Log("Beacon has no upgrade levels configured");
            return false;
        }

        if (currentUpgradeLevel >= upgradeLevels.Count)
        {
            Debug.Log("Beacon max level");
            return false;
        }

        UpgradeLevel nextLevel = upgradeLevels[currentUpgradeLevel];
        if (nextLevel == null)
            return false;

        if (!HasEnoughResources(inventory, nextLevel))
        {
            Debug.Log("Not enough resources");
            return false;
        }

        ConsumeResources(inventory, nextLevel);
    ...
    bool HasEnoughResources(Inventory inventory, UpgradeLevel level)
    {
        if (level.requiredResources == null || level.requiredResources.Length == 0)
            return true;

        for (int i = 0; i < level.requiredResources.Length; i++)
        {
            RequiredResource requirement = level.requiredResources[i];
            if (requirement == null || requirement.item == null)
                return false;

            int amount = Mathf.Max(1, requirement.amount);
            if (!inventory.HasItem(requirement.item, amount))
                return false;
        }
    ...
    void ConsumeResources(Inventory inventory, UpgradeLevel level)
    {
        if (level.requiredResources == null || level.requiredResources.Length == 0)
            return;
        ...
        for (int i = 0; i < level.requiredResources.Length; i++)
        {
            ...
            int amount = Mathf.Max(1, requirement.amount);
            inventory.Remove(requirement.item, amount);
        }
    }
