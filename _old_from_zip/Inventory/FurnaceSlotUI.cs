...
public class FurnaceSlotUI : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        ownerUI?.TryTransferFromFurnaceSlot(slotIndex, shiftPressed);
    }

    public void OnBeginDrag(PointerEventData eventData)
    {
        InventoryItem slot = ownerUI?.CurrentFurnace?.GetSlot(ToSlotType(slotIndex));
        ...
        if (draggedIcon == null)
        {
            draggedIcon = new GameObject("FurnaceDragIcon").AddComponent<Image>();
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        for (int i = 0; i < results.Count; i++)
        {
            ...
            if (targetFurnaceSlot == null)
                targetFurnaceSlot = result.gameObject.GetComponent<FurnaceSlotUI>();

            if (targetInventorySlot == null)
                targetInventorySlot = result.gameObject.GetComponent<InventorySlotUI>();

            if (targetFurnaceSlot != null || targetInventorySlot != null)
                break;
        }

        if (targetFurnaceSlot != null)
            ownerUI?.TrySwapFurnaceSlots(slotIndex, targetFurnaceSlot.Index);
        else if (targetInventorySlot != null)
            ownerUI?.TryDropFurnaceSlotToPlayerSlot(slotIndex, targetInventorySlot.Index);
