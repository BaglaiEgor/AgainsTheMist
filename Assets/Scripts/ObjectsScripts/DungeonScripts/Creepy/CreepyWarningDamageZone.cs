...
{
    private const int GeneratedCircleSize = 48;
    ...
    private void CacheVisualDefaults()
    {
        if (warningRenderer != null)
        {
            warningBaseColor = warningRenderer.color;
            warningBaseScale = warningRenderer.transform.localScale;
        }

        if (activeRenderer != null)
        {
            activeBaseColor = activeRenderer.color;
    ...
    private static Sprite GetGeneratedCircleSprite()
    {
        ...
        for (int y = 0; y < GeneratedCircleSize; y++)
        {
            for (int x = 0; x < GeneratedCircleSize; x++)
            {
                ...
                float fill = distance <= radius ? 0.28f : 0f;
                texture.SetPixel(x, y, new Color(1f, 1f, 1f, Mathf.Max(fill, edge)));
            }
        }
