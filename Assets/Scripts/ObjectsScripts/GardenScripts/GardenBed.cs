...
public class GardenBed : MonoBehaviour
{
    ...
    public bool HasPlantedCrop => plantedCrop != null && state != GardenBedState.Empty;
    public bool IsReadyToHarvest => state == GardenBedState.ReadyToHarvest && plantedCrop != null;
    ...
    public SaveGardenBedData CreateSnapshot()
    {
        return new SaveGardenBedData
        {
            position = SaveManager.ToSaveVector3(transform.position),
            cropId = plantedCrop != null ? plantedCrop.CropId : string.Empty,
            state = (int)state,
            growthMinutes = growthMinutes
        };
    }

    public void RestoreSnapshot(SaveGardenBedData data, CropDefinition crop)
    {
        if (data == null)
            return;

        state = (GardenBedState)Mathf.Clamp(data.state, 0, 2);
        plantedCrop = crop;
        growthMinutes = Mathf.Max(0, data.growthMinutes);
